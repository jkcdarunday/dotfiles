...
#define C(i,j) (inversesqrt(sqr(noise) + X[i].w + X[j].w) * exp(-0.5*(sqr(X[i].x - X[j].x)/(sqr(noise) + X[i].w + X[j].w) + sqr((coords[i] - coords[j])/radius))) + fixLuma * (X[i].x - c0.x) * (X[j].x - c0.x))
#define c(i) (inversesqrt(sqr(noise) + X[i].w + localVar) * exp(-0.5*(sqr(X[i].x - c0.x)/(sqr(noise) + X[i].w + localVar) + sqr((coords[i] - offset)/radius))))
...
vec4 hook() {
    ...
    for (i=0; i<N; i++) {
        ...
        for (int j=i; j<N; j++) {
            M[i*N + j] = C(i,j) - C(i,N) - C(j,N) + C(N,N);
        }
    }

    for (i=0; i<N; i++) {
        for (int j=i+1; j<N; j++) {
            b[j] -= b[i] * M[i*N + j] / M[i*N + i];
            for (int k=j; k<N; k++) {
                M[j*N + k] -= M[i*N + k] * M[i*N + j] / M[i*N + i];
            }
        }
    }
